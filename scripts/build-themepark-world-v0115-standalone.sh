...
case "$PRESET" in
  ...
  custom)
    ...
    BBOX="${TPMAP_BBOX:-}"
    [[ -n "$PARK_NAME" && -n "$BBOX" ]] || {
      echo "Custom builds require park_name and bbox." >&2
      exit 2
    }
    ;;
  *)
...
case "$ACCURACY" in
  ...
  verified)
    ...
    ;;
  *)
    echo "Unknown accuracy: $ACCURACY" >&2
    exit 2
    ;;
esac

SAFE_NAME="$(printf '%s' "$PARK_NAME" | tr '[:upper:]' '[:lower:]' | sed -E 's/[^a-z0-9]+/-/g; s/^-+|-+$//g')"
printf 'park=%s\nbbox=%s\naccuracy=%s\npath_geometry=%s\nsource=v0.11.5-standalone\n' \
  "$PARK_NAME" "$BBOX" "$ACCURACY_MODE" "$PATH_GEOMETRY_MODE" \
  | tee "$DIAG/parameters.txt"

[[ -f "$SOURCE_ZIP" ]] || {
  echo "Missing ThemePark Map v0.11.5 standalone source ZIP." \
    | tee "$DIAG/source-error.txt" >&2
  exit 2
}
...
[[ "$ACTUAL_SOURCE_SHA" == "$EXPECTED_SOURCE_SHA" ]] || {
  echo "v0.11.5 standalone source ZIP checksum mismatch." >&2
  exit 2
}
...
for file in \
  src/cli.mjs \
  src/lib/mcworld.mjs \
  src/lib/path-geometry.mjs \
  src/lib/path-topology.mjs \
  src/lib/orthophoto.mjs \
  src/lib/raster.mjs; do
  node --check "$GEN/$file"
done 2>&1 | tee "$DIAG/source-syntax-check.log"

node --input-type=module - "$GEN/package.json" <<'NODE' \
  2>&1 | tee "$DIAG/version-check.log"
import { readFileSync } from 'node:fs';
const file = process.argv[2];
const pkg = JSON.parse(readFileSync(file, 'utf8'));
if (pkg.version !== '0.11.5') {
  throw new Error(`Expected v0.11.5, got ${pkg.version}`);
}
...
if [[ -n "${TPMAP_ORTHOPHOTO_URL:-}" ]]; then
  ...
  curl --fail --location --retry 4 --retry-all-errors \
    --user-agent "ThemeParkMap/0.11.5 ($CONTACT)" \
    "$TPMAP_ORTHOPHOTO_URL" \
    -o "$GEN/data/park-orthophoto.tif" \
  ...
  [[ -n "${TPMAP_ORTHOPHOTO_DATE:-}" ]] && \
    IMAGERY_ARGS+=(--orthophoto-date "$TPMAP_ORTHOPHOTO_DATE")
else
  echo "No orthophoto supplied; whole-park non-OSM discovery is inactive." \
    | tee "$DIAG/orthophoto.txt"
fi
...
run_attempt() {
  ...
  rm -rf "$GEN/$OUT_REL"
  mkdir -p "$GEN/$OUT_REL"
  echo "strategy=$label elevation=$elevation overpass=$overpass dsm=$dsm" \
    | tee "$log"
...
node --input-type=module - "$VALIDATION_PATH" "$SUCCESS_STRATEGY" \
  "$(basename "$MCWORLD")" "$WORLD_SHA" "$WORLD_BYTES" <<'NODE'
