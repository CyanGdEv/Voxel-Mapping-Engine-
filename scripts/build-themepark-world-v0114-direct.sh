...
RUNTIME_SCRIPT="$RUNNER_TEMP/build-themepark-world-v0114-direct-runtime.sh"
...
PATCH_B64="$ROOT/patches/v0114-path-phase2a/v0114.patch.gz.b64"
PATCH_FILE="$DIAG/v0114-path-phase2a.patch"
[[ -f "$PATCH_B64" ]] || { echo "Missing v0.11.4 source patch." >&2; exit 2; }
[[ "$(sha256sum "$PATCH_B64" | cut -d' ' -f1)" == "d9d57ca10440770a944d95785b39c09a8991eb75119d1220c46908688bade556" ]] || {
  echo "v0.11.4 encoded patch checksum mismatch." >&2
  exit 2
}
base64 -d "$PATCH_B64" | gzip -dc > "$PATCH_FILE"
[[ "$(sha256sum "$PATCH_FILE" | cut -d' ' -f1)" == "3a6298f1c247d7b0cc7d8d549d99fe88e68e11a9430c010cef4c1a37a192ebb1" ]] || {
  echo "v0.11.4 decoded patch checksum mismatch." >&2
  exit 2
}
patch -d "$GEN" -p1 --batch --forward < "$PATCH_FILE" \
  2>&1 | tee "$DIAG/v0114-source-patch.log"
...
node --check "$GEN/src/lib/path-topology.mjs" 2>&1 | tee -a "$DIAG/source-syntax-check.log"
node --check "$GEN/src/lib/raster.mjs" 2>&1 | tee -a "$DIAG/source-syntax-check.log"
node --input-type=module - "$GEN/package.json" <<'NODE' 2>&1 | tee "$DIAG/version-check.log"
import { readFileSync } from 'node:fs';
const file = process.argv[2];
const pkg = JSON.parse(readFileSync(file, 'utf8'));
if (pkg.version !== '0.11.4') throw new Error(`Expected v0.11.4, got ${pkg.version}`);
