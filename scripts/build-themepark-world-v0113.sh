...
printf 'park=%s\nbbox=%s\naccuracy=%s\npath_geometry=%s\n' "$PARK_NAME" "$BBOX" "$ACCURACY_MODE" "$PATH_GEOMETRY_MODE" | tee "$DIAG/parameters.txt"
...
node "$ROOT/scripts/apply-v0112-direct-world-palette-fix.mjs" "$GEN" \
  2>&1 | tee "$DIAG/v0112-source-patch.log"

PATCH_B64="$DIAG/v0113-path-geometry-phase1.patch.gz.b64"
PATCH_FILE="$DIAG/v0113-path-geometry-phase1.patch"
cat "$ROOT"/patches/v0113-path-geometry-phase1/part-*.b64 > "$PATCH_B64"
[[ "$(sha256sum "$PATCH_B64" | cut -d' ' -f1)" == "39aeecf1c95c8aa83a876bc63f4e3bf98a228b06b94aa5f1ddd67802d39c436f" ]] || {
  echo "v0.11.3 encoded patch checksum mismatch." >&2
  exit 2
}
base64 -d "$PATCH_B64" | gzip -dc > "$PATCH_FILE"
[[ "$(sha256sum "$PATCH_FILE" | cut -d' ' -f1)" == "6392166bfbbc51e07496bc8e78b5ca5a9f43d5d6c1c2cad8148c47a2eba74425" ]] || {
  echo "v0.11.3 decoded patch checksum mismatch." >&2
  exit 2
}
patch -d "$GEN" -p1 --batch --forward < "$PATCH_FILE" \
  2>&1 | tee "$DIAG/v0113-source-patch.log"
...
if [[ -n "${TPMAP_ORTHOPHOTO_URL:-}" ]]; then
  ...
  curl --fail --location --retry 4 --retry-all-errors \
    --user-agent "ThemeParkMap/0.11.3 ($CONTACT)" \
