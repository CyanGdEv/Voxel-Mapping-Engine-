...
Phase 33 introduces a typed intermediate park scene between evidence fusion and Minecraft rasterization. The compiler now crosses a reconstruction-graph-owned boundary before rasterization, while the first cutover deliberately preserves the exact existing feature objects and ordering so block output remains unchanged until individual handlers are migrated safely.

## Why this exists

The previous pipeline moved directly from enriched GIS/planning features into a 1 m raster. That makes it difficult to solve cross-object 3D relationships such as ride supports meeting track and terrain, paths passing under rides, bridges crossing water, buildings adjoining paths, and barriers following circulation edges.
...
`reconstructionCompilerMap()` now owns the feature view passed to `compileMap`. In this first cutover it intentionally returns the exact original feature references, in the exact original order, after checking cardinality and identity. This establishes the reconstruction graph as the compiler boundary without changing existing block generation. Later Phase 33 slices can replace individual feature families with graph-derived compiler inputs under focused equivalence tests.

The graph-owned compiler references are non-enumerable and therefore do not inflate diagnostics.
...
1. Migrate selected compiler families from exact source-feature references to typed reconstruction nodes under output-equivalence tests.
