...
test('explicit coping adds one top course above DTM wall top',()=>{const c=compilation(),d=applyRetainingWallDetailToCompilation(c,graph({coping:'yes'}));assert.equal(d.explicitCapWalls,1);assert.ok(c.chunks.flatMap(ch=>ch.o).some(op=>op[0]===6&&op[1]===4&&op[2]===5&&op[3]===4));});
