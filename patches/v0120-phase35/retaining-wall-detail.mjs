// TPMAP_PHASE35_RETAINING_WALL_DETAIL_V2
...
import { structuralBlockFor, structuralMaterialFamily } from './structural-material-pattern-library.mjs';
...
export function applyRetainingWallDetailToCompilation(compilation, graph, options={}){
  ...
  const writes=new Map(); let wallsVisited=0,wallsDetailed=0,explicitThicknessWalls=0,explicitCapWalls=0,materialWalls=0,structuralCaps=0;
  for(const s of morphology.structures||[]){
    ...
    for(const cell of s.cells){
      ...
      const bodyBlock=material?.body||DEFAULT_BODY;
      for(const [x,z] of footprint){
        if(material||thickness){for(let y=y1;y<=y2;y++)setWrite(writes,{x,y,z,block:bodyBlock,kind:'body',wallId:s.id});}
        if(cap){setWrite(writes,{x,y:y2,z,block:capBlock(material?.family,x,y2,z,s.id),kind:'cap',wallId:s.id});structuralCaps++;}
      }
  ...
  const after=countOps(compilation),d={marker:'TPMAP_PHASE35_RETAINING_WALL_DETAIL_V2',status:'applied',phase:PHASE,datumM,wallsVisited,wallsDetailed,explicitThicknessWalls,explicitCapWalls,materialWalls,structuralCaps,writes:writes.size,emittedOperations:after-before,structuralMaterialLibrary:'TPMAP_PHASE35_STRUCTURAL_MATERIAL_PATTERN_LIBRARY_V1',policy:'dtm-vertical-truth;planning-explicit-thickness-cap-material-only;coping-uses-structural-slab-pattern-and-replaces-top-course-without-height-growth;no-generic-barrier-promotion;no-implicit-decoration'};
  compilation.meta.retainingWallDetail=d;const stats=compilation.stats||(compilation.stats={});stats.operations=after;stats.rawOperations=after;stats.retainingWallDetailWrites=writes.size;stats.retainingWallStructuralCaps=structuralCaps;return d;
}

export function validateRetainingWallDetailCompilation(compilation,diagnostics){
  if(!diagnostics||!['TPMAP_PHASE35_RETAINING_WALL_DETAIL_V1','TPMAP_PHASE35_RETAINING_WALL_DETAIL_V2'].includes(diagnostics.marker))throw new Error('Phase 35 retaining-wall detail diagnostics missing');
  ...
  if(diagnostics.marker==='TPMAP_PHASE35_RETAINING_WALL_DETAIL_V2'&&diagnostics.explicitCapWalls>0&&!diagnostics.structuralCaps)throw new Error('Phase 35 retaining-wall structural coping was lost');
  return compilation;
}
...
function explicitMaterial(tags){
  const raw=String(tags?.material||tags?.wall_material||'').toLowerCase().replaceAll('_','-');if(!raw)return null;
  const family=structuralMaterialFamily(raw,tags);if(!family)return null;
  let body=null;for(const [key,block] of Object.entries(LEGACY_BODY))if(raw.includes(key)){body=block;break;}
  if(!body)body=structuralBlockFor({material:family,role:'body',seed:0,featureId:`retaining-wall:${raw}`});
  return{family,body};
}
function capBlock(materialFamily,x,y,z,wallId){return structuralBlockFor({material:materialFamily||'stone',role:'cap',x,y,z,featureId:wallId});}
...
function noOp(datumM,wallsVisited){return{marker:'TPMAP_PHASE35_RETAINING_WALL_DETAIL_V2',status:'no-op',phase:PHASE,datumM,wallsVisited,wallsDetailed:0,explicitThicknessWalls:0,explicitCapWalls:0,materialWalls:0,structuralCaps:0,writes:0,emittedOperations:0,structuralMaterialLibrary:'TPMAP_PHASE35_STRUCTURAL_MATERIAL_PATTERN_LIBRARY_V1',policy:'no-explicit-planning-wall-detail-exact-no-op'};}
