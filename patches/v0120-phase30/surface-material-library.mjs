// ThemePark Map surface material library v1.
...
export function withThemeParkMaterialHints(style, feature = null) {
  ...
  const hint = firstText(base.materialPreset, base.material_preset, base.preset, base.material, base.materialClass, base.surface, base.name, base.label, base.role, base.appearanceStatus,
    feature?.surfaceMaterial, feature?.material, tags["tpmap:material"], tags["themepark:material"], tags.surface_material, tags["surface:material"], tags.surface, tags.material);
  if (!hint) return base;
  const preset = resolveThemeParkSurfaceMaterial({ ...base, materialPreset: hint });
  if (!preset) return base;
  return { ...base, materialPreset: preset.id, materialLabel: preset.label, palette: preset.palette, pattern: preset.pattern, schemaVersion: Math.max(2, Number(base.schemaVersion) || 0) };
}
