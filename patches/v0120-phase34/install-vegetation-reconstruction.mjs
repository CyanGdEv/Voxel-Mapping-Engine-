...
export function transformVerticalEngine(source){
  ...
  out=replaceOnce(out,'  diagnostics.terrainSurfaceModel = terrainSurfaceModel;','  diagnostics.terrainSurfaceModel = terrainSurfaceModel;\n  const vegetationReconstruction = reconstructVegetation(graph, options.verticalSources || null, options);\n  validateVegetationReconstructions(graph);\n  diagnostics.vegetationReconstruction = vegetationReconstruction;',"vegetation stage");
  validateVerticalEngine(out); return out;
}
function validateVerticalEngine(s){for(const t of ["TPMAP_PHASE34_VEGETATION_INTEGRATION","reconstructVegetation(graph, options.verticalSources || null, options)","validateVegetationReconstructions(graph)","diagnostics.vegetationReconstruction = vegetationReconstruction"])if(!s.includes(t))throw new Error(`Phase 34 vegetation integration missing ${t}`);const terrain=s.indexOf("buildTerrainSurfaceModel(graph, options.verticalSources || null, options)"),veg=s.indexOf("reconstructVegetation(graph, options.verticalSources || null, options)"),ride=s.indexOf("solveRideVerticalProfiles(graph, options)");if(!(terrain>=0&&veg>terrain&&(ride<0||ride>veg)))throw new Error("Phase 34 vegetation must run after terrain surfaces and before ride profiles");}
