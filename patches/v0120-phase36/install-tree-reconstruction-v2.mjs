...
const validateOnly=args.includes("--validate-only"),selfTest=args.includes("--self-test"),MARKER="TPMAP_PHASE36_TREE_RECONSTRUCTION_V2_INTEGRATION",NULL_MARKER="TPMAP_PHASE36_TREE_V2_NULL_EVIDENCE_SAFE";
...
async function install(root,validate){
  ...
  if(!validate){
    await writeFile(mod,normalizeTreeModule(await readFile(path.join(here,"tree-reconstruction-v2.mjs"),"utf8")));
  ...
  for(const token of ["TPMAP_PHASE36_TREE_RECONSTRUCTION_V2","tryCompileTreeReconstructionV2","maxLeafFillRatio","exactSpeciesFabricated: false",NULL_MARKER])if(!m.includes(token))throw new Error(`Tree V2 module missing ${token}`);
...
export function normalizeTreeModule(source){
  if(source.includes(NULL_MARKER))return source;
  const before='function firstFinite(...values){for(const value of values){const n=Number(value);if(Number.isFinite(n))return n;}return null;}';
  const after=`// ${NULL_MARKER}\nfunction firstFinite(...values){for(const value of values){if(value===null||value===undefined||value==="")continue;const n=Number(value);if(Number.isFinite(n))return n;}return null;}`;
  if(!source.includes(before))throw new Error("Tree V2 null-evidence anchor missing");
...
export function transformRaster(source){
  ...
  let depth=0,close=-1;for(let i=paren;i<out.length;i++){if(out[i]==="(")depth++;else if(out[i]===")"&&--depth===0){close=i;break;}}
  if(close<0)throw new Error("Tree V2 function parameter scan failed");let body=close+1;while(/\s/.test(out[body]))body++;if(out[body]!=="{")throw new Error("Tree V2 function body anchor missing");
...
function runSelfTest(){const sample='function compileVegetationFeature({add,feature}={}) { return null; }';const a=transformRaster(sample),b=transformRaster(a);if(a!==b)throw new Error("Tree V2 transform not idempotent");const tree=normalizeTreeModule('function firstFinite(...values){for(const value of values){const n=Number(value);if(Number.isFinite(n))return n;}return null;}');if(!tree.includes(NULL_MARKER))throw new Error("Tree V2 null safety self-test failed");validateRaster(a);console.log("Tree V2 installer self-test passed");}