...
// Preserve the existing provenance/status/licence gates, but stop discarding
// semantically classified, accurately georeferenced planning geometry solely
// because Phase 22's confidence scale tops out below Phase 23's old global gate.
...
async function install(root,validate){
  ...
  if(!validate){
    ...
    const after=transformFusion(before);
...
function validateFusion(source){
  for(const token of [MARKER,'planningFusionConfidenceGate(properties, confidence, rmseM, config)','effectiveMinConfidence','Math.min(config.minLineLengthM, 2)','Math.min(config.minAreaM2, 4)','hasPlanningSemanticEvidence(properties)']){
    if(!source.includes(token)) throw new Error(`Phase 36 planning fusion calibration missing ${token}`);
  }
...
function replaceOnce(source,before,after,label){
  ...
  if(i<0) throw new Error(`Phase 36 planning fusion calibration anchor missing: ${label}`);
  if(source.indexOf(before,i+before.length)>=0) throw new Error(`Phase 36 planning fusion calibration anchor ambiguous: ${label}`);
  return source.slice(0,i)+after+source.slice(i+before.length);
}

function selfTestTransform(){
  const sample=[
    'async function fuse(entry, properties, config, evidence) {',
    ...
    'const rejectedProvenance=true, rejectedLicense=true;'
  ].join('\n');
  const a=transformFusion(sample),b=transformFusion(a);
  if(a!==b) throw new Error('Phase 36 planning fusion calibration is not idempotent');
  validateFusion(a);
  console.log('Phase 36 planning fusion production calibration self-test passed');
}
